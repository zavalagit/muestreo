            ...
            @foreach ($especialidades->sortBy('nombre')->values() as $i => $especialidad)
               <tr>
                  ...
                  @foreach ($fiscalias->sortBy('nombre') as $j => $fiscalia)
                     <td> {{$peticiones->where('fiscalia2_id',$fiscalia->id)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','dictamen')->count()}} </td>
                  @endforeach
                  <td> {{$peticiones->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->where('documento_emitido','dictamen')->count()}} </td>
               </tr>
            @endforeach
            <tr>
               <td>{{$i+2}}</td>
               ...
               @foreach ($fiscalias->sortBy('nombre') as $fiscallia)
                  <td> {{$peticiones->where('fiscalia2_id',$fiscalia->id)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['informe','certicado'])->count()}} </td>
               @endforeach
               <td> {{$peticiones->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['informe','certicado'])->count()}} </td>
            </tr>
            {{-- <tr>
               <td>{{$i+3}}</td>
