            ...
            <tbody>
               ...
               @foreach ($especialidad->solicitudes->sortBy('nombre') as $solicitud)
                  <tr>
                     ...
                     @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                        <td>{{$peticiones->where('fiscalia2_id',$fiscalia->id)->where('solicitud_id',$solicitud->id)->where('documento_emitido','dictamen')->count()}}</td>
                     @endforeach
                     <td>{{$peticiones->where('solicitud_id',$solicitud->id)->where('documento_emitido','dictamen')->count()}}</td>
                  </tr>
               @endforeach
               ...
               <tr>
                  ...
                  <td>INFORMES</td>
                     @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                         <td>{{$peticiones->where('fiscalia2_id',$fiscalia->id)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['informe','certificado'])->count()}}</td>
                     @endforeach
                  <td>{{$peticiones->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['informe','certificado'])->count()}}</td>
               </tr>
