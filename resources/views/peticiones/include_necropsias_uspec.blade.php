...
@endphp
@foreach ($necropsias_uspec as $peticion)
         @php
...
<ul class="collapsible" data-collapsible="expandable" style="margin-top:0 !important;">
            ...
            <table class="highlight bordered">
               @foreach ($necropsias->where('necropsia_tipo',$key) as $necropsia)
                  <tr>
                     ...
                     <td class="td-der">{{$necropsias_uspec->where('solicitud_id',61)->where('necropsia_id',$necropsia->id)->count()}}</td>
                  </tr>
               @endforeach
            </table>
         </div>
   ...
   <li>
      <div class="row collapsible-header" style="padding:0 !important;">
         ...
         <div class="collapsible-der col s3"><b>{{$necropsias_uspec->where('solicitud_id',61)->count()}}</b></div>
      </div>
    </li>
</ul>
