      ...
      <tbody>
         ...
         @foreach ($especialidades->sortBy('nombre') as $especialidad)
               <tr>
                  ...
                  @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                     <td>{{ number_format( $peticiones->where('fiscalia2_id',$fiscalia->id)->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['dictamen','certificado'])->count() )}}</td>
                  @endforeach

               

                  <!--TOTAL-->   
                     <td>{{ number_format( $peticiones->whereIn('solicitud_id',$especialidad->solicitudes->pluck('id'))->whereIn('documento_emitido',['dictamen','certificado'])->count() )}}</td>
          ...
          @endforeach
            ...
            {{-- <tr>
               ...
               <td>Necropsias</td>
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                     <td>{{ number_format( $necropsias->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',2)->count() )}}</td>  
               @endforeach
               <!--TOTAL-->
            ...
            <tr>
               ...
               <td>Informes</td>
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                  <td>{{ number_format( $peticiones->where('fiscalia2_id',$fiscalia->id)->where('documento_emitido','informe')->count() )}}</td>                    
               @endforeach
               <!--TOTAL-->
               <td>{{ number_format( $peticiones->where('documento_emitido','informe')->count() )}}</td>  
            </tr>
            ...
            <tr>
               ...
               @endforeach
               <td>{{number_format( $peticiones->count() )}}</td>
            </tr>
            ...
            <tbody>
               ...
               <tr>
                  ...
                  @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                        <td>{{ number_format( $necropsias->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',2)->count() )}}</td>  
                  @endforeach
                  <td>{{ number_format( $necropsias->where('unidad_id',2)->count() )}}</td>
               </tr>
               <tr>
                  <td>{{$n++}}</td>
                  <td>USPEC</td>
                  @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                        <td>{{ number_format( $necropsias->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',6)->count() )}}</td>  
                  @endforeach
                  <td>{{ number_format( $necropsias->where('unidad_id',6)->count() )}}</td>
               </tr>
               <tr>
                  <td>{{$n++}}</td>
                  <td>UECCS</td>
                  @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                        <td>{{ number_format( $necropsias->where('fiscalia2_id',$fiscalia->id)->where('unidad_id',7)->count() )}}</td>  
                  @endforeach
                  <td>{{ number_format( $necropsias->where('unidad_id',7)->count() )}}</td>
               </tr>
