...
<body >
   ...
   <header>
    ...
    <main>
     ...
     <table>
       <caption><b>{{mb_strtoupper($necropsia_tipo)}}</b></caption>
      <thead>
          <tr>
            <th>No.</th>
      ...
      <tbody>
         @php $n = 1; @endphp
            @foreach ($necros->where('necropsia_tipo',$necropsia_tipo)->sortBy('nombre') as $necro)
               <tr>
                  <td>{{$n++}}</td>
                  <td style="text-align: 10% !important;">{{$necro->nombre}}</td>
            ...
            <tr>
              <td>{{$n++}}</td>
              <td>TOTAL</td>
              <td>{{$necropsias->where('fiscalia2_id',4)->where('unidad_id',2)->whereIn('necropsia_id',$necros->where('necropsia_tipo',$necropsia_tipo)->pluck('id'))->count()}}</td>
               <td>{{$necropsias->where('fiscalia2_id',4)->where('unidad_id',6)->whereIn('necropsia_id',$necros->where('necropsia_tipo',$necropsia_tipo)->pluck('id'))->count()}}</td>
               <td>{{$necropsias->where('fiscalia2_id',4)->where('unidad_id',7)->whereIn('necropsia_id',$necros->where('necropsia_tipo',$necropsia_tipo)->pluck('id'))->count()}}</td>
               @foreach ($fiscalias->sortBy('nombre') as $fiscalia)
                  @continue($fiscalia->id == 4)
                  <td>{{$necropsias->where('fiscalia2_id',$fiscalia->id)->whereIn('necropsia_id',$necros->where('necropsia_tipo',$necropsia_tipo)->pluck('id'))->count()}}</td>
                  @endforeach
                  <td>{{$necropsias->whereIn('necropsia_id',$necros->where('necropsia_tipo',$necropsia_tipo)->pluck('id'))->count()}}</td>

            </tr>
{{--
            @php
                $ids = [];
                      foreach ($necros->where('necropsia_tipo',$necropsia_tipo) as $key => $necro) {
