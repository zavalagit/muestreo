            ...
            <tbody>
               ...
               @foreach ($especialidad->solicitudes->sortBy('nombre') as $solicitud)
                  <tr>
                     ...
                     @foreach ($fiscalias as $fiscalia)
                        @if ( ($solicitud->id == 61) || ($solicitud->id == 62) )
                        <td>{{$necropsias->where('fiscalia2_id',$fiscalia->id)->where('solicitud_id',$solicitud->id)->count()}}</td>
                        @else
               ...
               <tr>
                  ...
                  <td>INFORMES</td>
                     @foreach ($fiscalias as $fiscalia)
                         <td>{{$peticiones->where('fiscalia2_id',$fiscalia->id)->whereIn('solicitud_id',$ids_solicitudes)->whereNotIn('solicitud_id',[61,62])->where('documento_emitido','informe')->count()}}</td>
                     @endforeach
                  <td>{{$peticiones->whereIn('solicitud_id',$ids_solicitudes)->whereNotIn('solicitud_id',[61,62])->where('documento_emitido','informe')->count()}}</td>
               </tr>
               <tr>
                  <td>{{$n++}}</td>
                  <td>TOTAL</td>
                  @foreach ($fiscalias as $fiscalia)
                     <td>{{$peticiones->where('fiscalia2_id',$fiscalia->id)->whereIn('solicitud_id',$ids_solicitudes)->whereNotIn('solicitud_id',[61,62])->whereIn('documento_emitido',['dictamen','certificado','informe'])->count()}}</td>
                     @endforeach
                     <td>{{ number_format($peticiones->whereIn('solicitud_id',$ids_solicitudes)->whereNotIn('solicitud_id',[61,62])->whereIn('documento_emitido',['dictamen','certificado','informe'])->count() ) }}</td>
               </tr>
            </tbody>   
         </table>
