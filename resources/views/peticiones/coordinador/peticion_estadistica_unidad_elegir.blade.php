      ...
      @foreach ($unidades as $unidad)
         <div class="col s3">
            <div class="div-fiscalia">
               ...
               <p>solicitudes: {{$peticiones->where('unidad_id',$unidad->id)->count()}}</p>
               <p>solicitudes atendidas: {{$atendidas->where('unidad_id',$unidad->id)->wherein('estado',['atendida','entregada'])->count()}}</p>
               <a href="/peticion-estadistica/unidad/{{$unidad->id}}?fecha_inicio={{$fecha_inicio}}&fecha_fin={{$fecha_fin}}&buscar_btn=buscar">Ver más...</a>
            </div>
         </div>
      @endforeach
   </div>
