 ...
 <table class="tabla-modal bordered">
  ...
  <!--nuc-->
   ...
   <tr>
    ...
    <td>
      @isset($peticion->oficio_numero)
        {{$peticion->oficio_numero}}
      @endisset
      @empty($peticion->oficio_numero)
        ---
      @endempty
   </td>
 </tr>
 <tr>
   ...
   <tr>
     <th>{{$n++}}.- SOLICITUD</th>
     <td>
        {{mb_strtoupper($peticion->solicitud->nombre)}}
      </td>
   </tr>
   @if (in_array($peticion->solicitud_id,[61,62]))
    <tr>
...
<tr>
  ...
  <td>
    {{-- {{$peticion->unidad2->nombre ?? '---'}} --}}
    {{isset($peticion->unidad2_id) ? $peticion->unidad2->nombre : '---'}}
 </td>
</tr>
