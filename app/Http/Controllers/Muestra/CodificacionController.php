...
class CodificacionController extends Controller
{
   ...
   public function create(Request $request)
   {
      ...
      //  set_time_limit(0);
      if ($request->has('btn') && ($request->filled('ci_pp'))) {
         $cadenas = Cadena::where('ci_pp','like',"%{$request->ci_pp}%")
                           ->with('indicios')
                           ->get();
      } 

      $request->flash();
      return view('muestreo1.codificacion.codificacion_multipleindicios_formulario', [
         'formAccion' => $formAccion,
         'cadenas' => isset($cadenas) ? $cadenas : null,
